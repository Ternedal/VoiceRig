...
def test_warm_diarization_uses_preload_protocol_and_privacy_default(tmp_path: Path, monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        ...
        return SimpleNamespace(
            returncode=0,
            stdout="VOICERIG_DIARIZATION_READY=" + json.dumps(_diarization_payload()) + "\n",
            stderr="",
        )
    ...
    assert report["torchcodec_version"] == DIARIZATION_TORCHCODEC_VERSION
...
def test_warm_diarization_rejects_unverified_package_version(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: SimpleNamespace(
            returncode=0,
            stdout="VOICERIG_DIARIZATION_READY=" + json.dumps(_diarization_payload(package_version="4.9.9")) + "\n",
            stderr="",
        ),
    )
...
def test_warm_diarization_rejects_wrong_torchcodec_generation(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: SimpleNamespace(
            returncode=0,
            stdout="VOICERIG_DIARIZATION_READY=" + json.dumps(_diarization_payload(torchcodec_version="0.13.0")) + "\n",
            stderr="",
        ),
    )
...
def test_readiness_marker_records_exact_model_contract(tmp_path: Path, monkeypatch):
    ...
    report = {
        "ok": True,
        "chatterbox": {"ok": True},
        "diarization": _diarization_payload(),
    }
